...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_images_path = glob(os.path.join(instance_data_root, '*.png'))
        print( 'line427, number of image: {}'.format( len( self.instance_images_path ) ) )

        # self.instance_images_path = list(Path(instance_data_root).iterdir())

        ### CHANGE LATER: I wanted to limit the number of instance images 
        self.num_instance_images = len(self.instance_images_path)
        # self.num_instance_images = 50
...
def main(args):
    ...
    if accelerator.device.type == "cuda":
        logging.info("CUDA is available and Accelerator is using CUDA.")
    else:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
            ...
            logging.info(f"Validation check: Prompt={args.validation_prompt}, Epoch={epoch}, Validation Epochs={args.validation_epochs}")
            ...
            for i, image in enumerate(images):
                image.save(os.path.join(validation_dir, f"epoch_{epoch}_image_{i}.png"))
